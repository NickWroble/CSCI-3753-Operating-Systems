...
void push(stack_struct *stack, char *name){
	//printf("Push %s\n", name);
	...
	strcpy(stack->array[stack->top++], name);
	...
	while((*((pthread_create_request_args*)ptr)->num_files - (*((pthread_create_request_args*)ptr)->fp)) > 0){
			...
			if(fp){
				while(fscanf(fp, "%s\n", hostname) != EOF){
					...
					push(((pthread_create_request_args*)ptr)->stack, hostname);
				}
...
void* consumer(void *ptr){ 
	...
	int num_resolved = 0;
		while (1){
			hostname = pop(((pthread_create_resolve_args*)ptr)->stack);	
			if(!hostname){
				break;
			}
			char ip_addr[MAX_IP_LENGTH];
			sem_wait(((pthread_create_resolve_args*)ptr)->dnslookup_mutex);
			...
			sem_post(((pthread_create_resolve_args*)ptr)->dnslookup_mutex);
...
//int main(){
	...
	sem_t stdout_mutex, args_mutex;//, resolve_file, request_file;//, resolve_mutex, request_mutex;
	...
	for(int i = 0; i < num_requestor; i++){
		...
		request_args[i].dnslookup_mutex	= &dnslookup_mutex;
	...
	for(int i = 0; i < num_resolver; i++){
		...
		resolve_args[i].dnslookup_mutex	= &dnslookup_mutex;
